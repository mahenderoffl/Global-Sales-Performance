...
def download_real_data():
    """Get the real Global Tech Gadget Consumption dataset from local file."""
    try:
        # Path to your local dataset
        csv_file = os.path.join("data", "Global_Tech_Gadget_Consumption.csv")
...
@st.cache_data
def download_real_data_2():
    """Get the second real dataset from local file."""
    try:
        # Check for Excel files first, then CSV
        excel_extensions = ['.xlsx', '.xls']
        csv_file = os.path.join("data", "Real_Dataset_2.csv")
        
        # Look for Excel files
        for ext in excel_extensions:
            excel_file = os.path.join("data", f"Real_Dataset_2{ext}")
        ...
        with st.expander("� Troubleshooting Tips"):
            ...
            **Possible solutions:**
            ...
            2. **Kaggle Authentication**: Make sure you have Kaggle API credentials set up
            3. **Dataset Access**: Verify the dataset exists and is publicly accessible
            4. **Firewall/Proxy**: Check if your network allows downloads from Kaggle
            
            **Setup Kaggle API:**
            1. Go to kaggle.com → Account → API → Create New API Token
            2. Download kaggle.json file
            3. Place it in ~/.kaggle/ directory (or C:\\Users\\{username}\\.kaggle\\ on Windows)
            """)
        return None
...
def sidebar_content():
    ...
    data_source = st.radio(
        ...
        ["Use Sample Data", "Use Real Data", "Use Real Dataset 2", "Upload File", "Paste CSV Data"],
        index=0
    )
    ...
    if data_source == "Upload File":
        ...
        st.markdown("**🌍 Real Dataset from Kaggle:**")
        st.info("📊 Global Tech Gadget Consumption Data (2015-2025)")
        
        # Add description of the real dataset
        with st.expander("📋 About This Dataset"):
            st.markdown("""
            **🔍 Dataset Details:**
            - **Source**: Kaggle - Global Tech Gadget Consumption Data
            - **Period**: 2015-2025
            - **Coverage**: Global technology gadget sales and consumption patterns
            - **Features**: Country-wise sales data, product categories, regional analysis
            
            **🚀 Benefits:**
            - Real-world data for accurate insights
            - Large dataset with comprehensive coverage
            - Multiple years of historical data
            - Professional-grade analysis ready data
            """)
        
        if st.button("📥 Use Real Dataset", type="primary"):
            real_data_path = download_real_data()
            if real_data_path:
                st.session_state['real_data_path'] = real_data_path
                st.success("✅ Real dataset loaded successfully!")
                st.rerun()
            else:
                st.error("❌ Could not find the real dataset file. Please ensure 'Global_Tech_Gadget_Consumption.csv' is in the 'data' folder.")
    
    elif data_source == "Use Real Dataset 2":
        st.markdown("**🍫 Real Dataset 2:**")
        st.info("� Global Chocolate Sales Data (2022)")
        
        # Add description of the second real dataset
        with st.expander("📋 About This Dataset"):
            st.markdown("""
            **🔍 Dataset Details:**
            - **Source**: Global Chocolate Sales Database
            - **Period**: 2022 sales data
            - **Coverage**: International chocolate product sales across multiple countries
            - **Features**: Sales persons, countries, chocolate products, dates, amounts, and shipment data
            
            **🚀 Benefits:**
            - Real chocolate industry sales data
            - Multiple product categories (Dark Bars, Peanut Butter Cubes, Mint Chip, etc.)
            - International market coverage (UK, India, Australia, New Zealand, etc.)
            - Professional sales analysis ready
            """)
        
        if st.button("📥 Use Real Dataset 2", type="primary"):
            real_data_path_2 = download_real_data_2()
            if real_data_path_2:
                st.session_state['real_data_path_2'] = real_data_path_2
                st.success("✅ Chocolate Sales Dataset loaded successfully!")
                st.rerun()
            else:
                st.error("❌ Could not find Real Dataset 2. Please ensure 'Real_Dataset_2.csv' is in the 'data' folder.")
...
def display_dashboard_content(processor, visualizer, sample_data_path, data_source, uploaded_file, csv_text, device_type):
    ...
    if data_source == "Upload File" and uploaded_file is not None:
        ...
        # Process real dataset from Kaggle
        if 'real_data_path' in st.session_state and st.session_state['real_data_path']:
            try:
                real_data_path = st.session_state['real_data_path']
                data_dict = processor.process_full_pipeline(real_data_path)
            except Exception as e:
                st.error(f"❌ Error processing real dataset: {str(e)}")
                st.info(" Using sample data instead.")
                data_dict = processor.process_full_pipeline(sample_data_path)
        else:
            st.warning("⚠️ Please load the real dataset first using the button above.")
            st.info("🔄 Using sample data meanwhile...")
            data_dict = processor.process_full_pipeline(sample_data_path)
    
    elif data_source == "Use Real Dataset 2":
        # Process second real dataset
        if 'real_data_path_2' in st.session_state and st.session_state['real_data_path_2']:
            try:
                real_data_path_2 = st.session_state['real_data_path_2']
                data_dict = processor.process_full_pipeline(real_data_path_2)
            except Exception as e:
                st.error(f"❌ Error processing Real Dataset 2: {str(e)}")
                st.info("💡 Using sample data instead.")
                data_dict = processor.process_full_pipeline(sample_data_path)
        else:
            st.warning("⚠️ Please load Real Dataset 2 first using the button above.")
            st.info("🔄 Using sample data meanwhile...")
            data_dict = processor.process_full_pipeline(sample_data_path)
...
def create_filter_content(data_dict, use_sidebar=False):
    ...
    if 'Year' in data_dict['cleaned_data'].columns:
        # Filter out NaN values and get unique years
        valid_years = data_dict['cleaned_data']['Year'].dropna().unique()
        if len(valid_years) > 0:
            available_years = sorted([int(year) for year in valid_years])
            ...
            widget_func.info("No valid year data available for filtering")
...
def display_visualizations(visualizer, filtered_data_dict, data_dict, data_source, device_type):
    ...
    ">
        <strong>Built using Streamlit • Python • Pandas • Plotly • NumPy</strong><br>
        <span style="font-size: 16px;">👨‍💻 Developed by <strong>Mahender Banoth (IIT Patna)</strong></span><br>
        <a href="https://instagram.com/mahender_hustles" target="_blank" style="color: #ffeb3b; text-decoration: none; font-weight: bold; font-size: 16px;">
            📷 @mahender_hustles
        </a>
    </div>
    """, unsafe_allow_html=True)
